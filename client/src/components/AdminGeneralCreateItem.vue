  ...
  <v-card>
    ...
    <v-card-text>
      <form @submit.prevent="submit">
        <v-checkbox v-model="allowEditPassword" label="Alterar senha?"
          v-if="props.edit && currentName === 'Admin'"></v-checkbox>
        <v-text-field v-model="name.value.value" :counter="300" :error-messages="name.errorMessage.value"
          label="Nome"></v-text-field>
        <v-text-field v-model="password.value.value" :append-icon="show ? 'mdi-eye' : 'mdi-eye-off'"
          :type="!show ? 'text' : 'password'" label="Senha" name="input-10-1" counter @click:append="show = !show"
          v-if="!props.edit && currentName === 'Admin'" :error-messages="password.errorMessage.value"></v-text-field>
        <v-text-field v-model="password.value.value" :append-icon="show ? 'mdi-eye' : 'mdi-eye-off'"
          :type="show ? 'text' : 'password'" label="Nova senha" name="input-10-1" counter @click:append="show = !show"
          v-if="props.edit && currentName === 'Admin' && allowEditPassword" :disabled="!allowEditPassword"
