<template>
  <v-data-table class="data-table" :headers="headers" :items="filteredUsers" v-if="!loading" :items-per-page="25"
    :items-per-page-options="[20, 50, 100]"
    style="font-size: 1em; overflow-y: auto; max-width: 1500px; min-width: 80%; width: 100%; max-height: 700px;"
    itemsPerPageText="Usuários por página" :loading="loading" item-value="name">
    <template v-slot:top>
        ...
        <v-dialog v-model="dialogRestore" max-width="30%">
          <v-card>
            ...
            <v-card-actions>
              ...
              <v-btn color="green" variant="text" @click="restoreItemConfirm"
                :loading="loadingRestore">Confirmar</v-btn>
              <v-spacer></v-spacer>
            </v-card-actions>
          </v-card>
        </v-dialog>
    ...
    <template v-slot:item.actions="{ item }">
      <div style="display: flex; gap: 2px; align-items: center;">
        ...
        <div style="display: flex; flex-direction: column; align-items: center;">
          <v-icon size="small" @click="restoreItem(item)">
            mdi-restore
          </v-icon>
