<template>
  <v-card class="mx-auto" min-width="450px" max-width="450px">
    ...
    <v-container>
      <v-form @submit.prevent="submit">
        <v-text-field v-model="name.value.value" label="Nome" :error-messages="name.errorMessage.value"
          clearable></v-text-field>
        <v-text-field v-model="email.value.value" label="Email" type="email" :error-messages="email.errorMessage.value"
          clearable></v-text-field>
        ...
        <v-container fluid>
          <v-row justify="start">
            <v-col cols="auto">
              <v-checkbox v-model="tcc" :label="`TCC?`"></v-checkbox>
            </v-col>
            <v-col cols="auto">
              <v-checkbox v-model="tur" :label="`TUR?`"></v-checkbox>
            </v-col>
            <v-col>
              <v-text-field v-if="tcc || tur" v-model="sid.value.value" label="SID termos" clearable counter="12"
                :error-messages="sid.errorMessage.value"></v-text-field>
              <v-text-field v-else v-model="sid.value.value" label="SID termos" clearable counter="12"
                :error-messages="sid.errorMessage.value" disabled></v-text-field>
            </v-col>
          </v-row>
        </v-container>
        <v-btn :loading="loading" type="submit" size="large" block color="primary" variant="tonal"
          class="mt-2">Cadastrar</v-btn>
      </v-form>
    </v-container>
  </v-card>
</template>
  ...
  try {
    ...
    await axios.post(`${globalState.apiUrl.value}/api/user`, userData, {
      ...
      .then(response => {
        ...
        toastr.error('Erro ao criar usuário, talvez já exista outro usuário com mesmo email', null, { timeOut: 1500 });
      });
  } catch (e) {
    toastr.error('Erro ao criar usuário, talvez já exista outro usuário com mesmo email', null, { timeOut: 1500 });
  }
