<template>
  <v-data-table-virtual :headers="headers" :items="items.sort((a, b) => {
    ...
    return 0;
  })" item-value="name" height="400px" :loading="loading">
    <template v-slot:top>
      <div class="d-flex align-items-center mb-3 mt-5">
        ...
        <div>
          ...
          <v-tooltip activator="parent" location="top">Adicionar</v-tooltip>
        </div>
      </div>
      ...
      <v-dialog v-model="dialogDelete" max-width="30%">
        <v-card>
          ...
          <p style="text-align: center;" v-if="!allowedToDelete">Não é possível deletar o {{ headers[0].title }} porque
            existem usuários associados a ele.</p>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="blue-darken-1" variant="text" @click="toggleDeleteDialog">Cancelar</v-btn>
            <v-btn color="red-darken-3" variant="text" :loading="loadingDelete" :disabled="!allowedToDelete"
              @click="confirmDelete()">Confirmar</v-btn>
            <v-spacer></v-spacer>
          </v-card-actions>
        </v-card>
      </v-dialog>
